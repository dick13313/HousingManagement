...
## Supabase Auth 驗證信 URL

註冊時前端會把 `emailRedirectTo` 設為使用者目前所在站台的 origin，例如：

- `https://your-site.netlify.app/`
- `https://deploy-preview-123--your-site.netlify.app/`

請到 Supabase Dashboard 的 `Authentication -> URL Configuration` 設定：

| 欄位 | 建議值 |
| --- | --- |
| `Site URL` | Netlify 正式站 URL |
| `Redirect URLs` | Netlify 正式站 URL、`http://localhost:3000/**`，以及需要使用 preview deploy 時的 `https://**--your-site.netlify.app/**` |

若 Supabase 沒有允許 `emailRedirectTo` 指定的 URL，驗證信可能會退回 `Site URL`，
導致從 Netlify 註冊的新帳號收到錯誤或 localhost 的驗證連結。

若有自訂 Supabase 驗證信 template，確認連結應使用 `{{ .RedirectTo }}`，不要只寫死
`{{ .SiteURL }}`。
